 ...
 *
 * <li>An <b>unbounded wildcard</b> (the {@code ?} in a type
 * like {@code List<?>}, not followed by {@code extends} or
 * {@code super}) represents <i>any</i> type within the type
 * parameter's bounds with no further restriction. A
 * <b>lower-bounded wildcard</b> ({@code List<? super
 * String>}) similarly has no <i>upper</i> bound of its own.
 ...
 * extends Object>} have always been identical as base types,
 * they are no longer identical as
 * <a href="Nullable.html#augmented-types">augmented types</a>.
 * (<a href="https://bit.ly/3ppb8ZC">Why?</a>)
 *
 *   <ul>
 *   <li>Conversely, a <b>type parameter</b> is always bounded:
 *   when none is given explicitly, {@code Object} is filled in
 ...
 * type arguments safely, the {@code E} type itself must be
 * handled <i>strictly</i>: as if nullable when "read from",
 * but as if non-null when "written to". (Contrast with
 * {@code class Foo<E extends Bar>}, where usages of {@code E}
 * are simply non-null, just like usages of {@code String}.)
 *
 ...
 * It is possible for different versions of
 * {@code package-info} to be visible in different analysis
 * scenarios, which could cause the same single class to be
 * interpreted inconsistently. As one example, a
 * {@code package-info} file from a "test" source root might
 * hide the one from the "main" source root.
 */
@Documented
